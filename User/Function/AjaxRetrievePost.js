$(document).ready(function() {
    ...
    function loadPosts(PostFilter) {
        ...
        $.ajax({
            url: '../User/Function/RetrivePost.php', //Function file
            type: 'GET', //Server request
            data: { // data pass to function
                page: Page,  
                postFilter: PostFilter
            },
            success: function(response) { //Success block  with function holding response from function file
                ...
                if (response.count < 5) { //Check if there still more content to load
    ...
    $('#MainBody').on("scroll", function() {
        ...
        if (!bottomReach && scrollTop + divHeight >= scrollHeight - 1) { //curentbottomposition >= absolute bottom 
            bottomReach = true
            loadPosts(PostFilter);
