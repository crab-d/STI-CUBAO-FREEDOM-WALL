...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $htmlContent .= '
        <div class="bg-light rounded-2 shadow-sm border m-2 p-0 w-75 d-flex flex-column overflow-hidden container flex-shrink-0">
                ...
                <div id="PostCard_ActionBar" class="rounded bg-white d-flex justify-content-between p-2">
                    <p class="m-0"> Likes <span class="rounded text-white poppins-medium primary-color p-1" style"height:10px; width:10px;>' . $row['like_count'] . '</p>
                    <p class="m-0">' . $row['comment_count'] . '</p>
                </div>
        </div>
    ';
}
